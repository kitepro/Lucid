<?php    
    ...
    //GET INFO
    if($show){
        $chkuser = $con->prepare("SELECT GRNumber,Name,BE_Division,RollNum,email,mydet,alias FROM studentlist WHERE id=?");
        $chkuser->bind_param("i",$_GET['sid']);
        $chkuser->execute(); 
        $info = $chkuser->get_result()->fetch_assoc();
    }
    else{
        $_SESSION['homesess'] = "You'll Get The Info, Please Login First";
        header('location:/BEProjects/Home/');
        exit;
    }
...
<html lang="en">
    ...
    <body style="margin: 0px;padding: 0px;" onload="document.getElementById('loader').style.display = 'none';">
        ...
        <br><br>
        <label class="infos">GRNumber : </label>
        <label class="texts">
        <?php            
            echo htmlspecialchars($info['GRNumber']);
        ?>
        </label>
        <br><br>
        <label class="infos">Roll No : </label>
        <label class="texts">
        <?php         
            echo htmlspecialchars($info['RollNum']);   
        ?>
        </label>
        <br><br>
        <label class="infos">Division : </label>
        <label class="texts">
        <?php            
            echo htmlspecialchars($info['BE_Division']);   
        ?>
        </label>
        <br><br>
        ...
        <label class="texts">
        <?php    
            echo htmlspecialchars($info['email']);         
        ?>
        </label>
        <br><br>
        <?php   
            if(isset($info['alias']) && $info['alias']!=NULL) echo '<label class="infos">Alias : </label><label class="texts">'.htmlspecialchars($info['alias']).'</label>';          
        ?>
        <br><br>
        <?php   
            if(isset($info['mydet']) && $info['mydet']!=NULL) echo '<label class="infos">Details : </label><label class="texts">'.htmlspecialchars($info['mydet']).'</label>';          
        ?>
        <br><br>
        </div>
        <div style="position: absolute;border: 2px solid #000;height: 300px;width: 300px;z-index: 100;right: 200px;top: 150px;text-align: center;font-family: 'Comic Sans MS';line-height: 300px;">
            <?php                
                $con = mysqli_connect("localhost","root","","viitbe");
                if(isset($_GET['sid'])){
                    $chkuser = $con->prepare("SELECT pimage FROM studentlist WHERE id=?");
                    $chkuser->bind_param("i",$_GET['sid']);
                }               
                $chkuser->execute(); 
